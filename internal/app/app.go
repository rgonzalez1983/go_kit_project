...
import (
	...
	repository "go_kit_project/internal/repository"
...
type App struct {
	...
	PersonMiddleware middleware.PersonMiddleware
}
...
func (a *App) InitializeRoutes() {
	...
	a.Router.Methods(http.MethodPost).Path(static.URLDeletingOne + "/{" + static.KeyId + "}").Handler(a.DeletePerson(options))
}
...
func (a *App) InitializeEndpoints() {
	a.PersonRepository = repository.NewPersonRepository(a.DB)
	a.PersonService = service.NewPersonService(a.PersonRepository, a.Logg)
	a.PersonMiddleware = middleware.NewPersonMiddleware(a.Logg)
	a.PersonEndpoints = endpoints.MakePersonEndpoints(&a.PersonService, a.PersonMiddleware)
}
